      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'clamp(40px,6vw,80px)', alignItems: 'start' }}>
          <div>
            ...
            <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 700, color: '#f0f4fa', marginBottom: '20px', lineHeight: 1.15, letterSpacing: '-0.02em' }}>Practical Freight Solutions by Sea</h2>
            <div style={{ width: '48px', height: '2px', background: 'var(--gold)', marginBottom: '24px' }}></div>
            <p style={{ color: 'var(--muted)', fontSize: 'clamp(15px,1.8vw,17px)', lineHeight: 1.85, marginBottom: '32px' }}>
            ...
            <div style={{ display: 'flex', gap: 'clamp(24px,4vw,48px)', flexWrap: 'wrap' }}>
              {[['8','Commodity Types'],['4+','Vessel Classes'],['Global','Port Coverage']].map(([n,l])=>(
                <div key={l}>
                  ...
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--faint)', marginTop: '4px' }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
          <div>
            {steps.map(s=>(
              <div key={s.n} style={{ display: 'flex', gap: '20px', padding: '22px 0', borderBottom: '1px solid var(--border)' }}>
                ...
                <div>
                  ...
                  <div style={{ color: 'var(--muted)', fontSize: '15px', lineHeight: 1.7 }}>{s.body}</div>
                </div>
              </div>
            ))}
          </div>
