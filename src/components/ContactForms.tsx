...
export default function ContactForms() {
  ...
  return (
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1px', background: 'var(--border)', marginBottom: '1px' }}>
        {tabs.map((t, i) => (
          <button key={t} type="button" onClick={() => { setActive(i); setStatus('idle'); }}
            style={{ background: active===i?'var(--ocean)':'var(--navy2)', color: active===i?'var(--gold2)':'var(--faint)', fontFamily: 'var(--font-mono)', fontSize: '11px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '12px 16px', border: 'none', cursor: 'pointer', flex: '1 1 auto' }}>
            {t}
          </button>
        ))}
      </div>
