  ...
  return (
    <section style={{ padding: 'clamp(48px,8vw,96px) 0', background: 'var(--navy)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(1.6rem,4vw,2.8rem)', fontWeight: 600, color: '#f0f4fa', marginBottom: '12px' }}>Vessel Coverage</h2>
        <p style={{ color: 'var(--muted)', fontSize: '15px', lineHeight: 1.8, maxWidth: '640px', marginBottom: '32px' }}>NOEMA GROUP coordinates suitable vessel capacity based on cargo type, quantity, port restrictions, laycan, and route requirements.</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1px', background: 'var(--border)' }}>
          {vessels.map(v=>(
            <div key={v.name} style={{ background: 'var(--navy3)', padding: '24px 20px', borderBottom: '3px solid transparent', transition: 'border-color 0.2s' }}>
